...
def setup_logging(
    ...
    Args:
        level: Log level (DEBUG, INFO, WARNING, ERROR, CRITICAL).
               Defaults to LEDGERLOOP_LOG_LEVEL env var or INFO.
        json_format: If True, use JSON formatter. Defaults to LEDGERLOOP_LOG_JSON env var.
        log_file: Optional file path to write logs to.
    """
    # Determine log level
    if level is None:
        level = os.getenv("LEDGERLOOP_LOG_LEVEL", "INFO").upper()

    # Determine format
    if json_format is False:
        json_format = os.getenv("LEDGERLOOP_LOG_JSON", "false").lower() == "true"
    ...
    if log_file or os.getenv("LEDGERLOOP_LOG_FILE"):
        file_path = log_file or os.getenv("LEDGERLOOP_LOG_FILE")
...
def get_logger(name: str) -> logging.Logger:
    ...
    Usage:
        from ledgerloop.logging_config import get_logger
