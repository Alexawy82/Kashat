...
class TransferDetectionConfig:
    ...
    def from_env(cls) -> "TransferDetectionConfig":
        ...
        import os
        return cls(
            max_days=int(os.getenv("LEDGERLOOP_TRANSFER_MAX_DAYS", "3")),
            amount_tolerance_pct=float(os.getenv("LEDGERLOOP_TRANSFER_AMOUNT_TOL", "0.02")),
            min_heuristic_score=float(os.getenv("LEDGERLOOP_TRANSFER_MIN_SCORE", "0.5")),
            ai_confidence_threshold=float(os.getenv("LEDGERLOOP_TRANSFER_AI_THRESHOLD", "0.75")),
            enable_ai_analysis=os.getenv("LEDGERLOOP_TRANSFER_ENABLE_AI", "true").lower() == "true",
        )
