...
class AIConfig:
    ...
    def from_env(cls) -> "AIConfig":
        ...
        return cls(
            provider=os.getenv("LEDGERLOOP_AI_PROVIDER", str(s.get("ai_provider", "auto"))).lower(),
            openai_api_key=os.getenv("OPENAI_API_KEY", str(s.get("ai_openai_api_key", ""))) or None,
            openai_base_url=os.getenv("LEDGERLOOP_AI_OPENAI_BASE_URL", str(s.get("ai_openai_base_url", ""))) or None,
            openai_model=os.getenv("LEDGERLOOP_AI_OPENAI_MODEL", str(s.get("ai_openai_model", "gpt-4o-mini"))),
            lmstudio_base_url=os.getenv("LEDGERLOOP_AI_LMSTUDIO_BASE_URL", str(s.get("ai_lmstudio_base_url", "http://127.0.0.1:1234/v1"))),
            lmstudio_api_key=os.getenv("LEDGERLOOP_AI_LMSTUDIO_API_KEY", "lm-studio"),
            lmstudio_model=os.getenv("LEDGERLOOP_AI_LMSTUDIO_MODEL", str(s.get("ai_lmstudio_model", ""))) or None,
            timeout=float(os.getenv("LEDGERLOOP_AI_TIMEOUT", str(s.get("ai_timeout", "30")))),
            connect_timeout=float(os.getenv("LEDGERLOOP_AI_CONNECT_TIMEOUT", str(s.get("ai_connect_timeout", "5")))),
            max_retries=int(os.getenv("LEDGERLOOP_AI_MAX_RETRIES", str(s.get("ai_max_retries", "2")))),
            retry_min_wait=float(os.getenv("LEDGERLOOP_AI_RETRY_MIN_WAIT", str(s.get("ai_retry_min_wait", "0.5")))),
            retry_max_wait=float(os.getenv("LEDGERLOOP_AI_RETRY_MAX_WAIT", str(s.get("ai_retry_max_wait", "10")))),
            retry_multiplier=float(os.getenv("LEDGERLOOP_AI_RETRY_MULTIPLIER", str(s.get("ai_retry_multiplier", "2")))),
            retry_jitter=os.getenv("LEDGERLOOP_AI_RETRY_JITTER", str(s.get("ai_retry_jitter", "true"))).lower() == "true",
            max_concurrency=int(os.getenv("LEDGERLOOP_AI_MAX_CONCURRENCY", str(s.get("ai_max_concurrency", "2")))),
            requests_per_minute=int(os.getenv("LEDGERLOOP_AI_REQUESTS_PER_MINUTE", str(s.get("ai_requests_per_minute", "60")))),
            batch_size=int(os.getenv("LEDGERLOOP_AI_BATCH_SIZE", str(s.get("ai_batch_size", "5")))),
            confidence_threshold=float(os.getenv("LEDGERLOOP_AI_CONFIDENCE_THRESHOLD", str(s.get("ai_confidence_threshold", "0.7")))),
            temperature=float(os.getenv("LEDGERLOOP_AI_TEMPERATURE", str(s.get("ai_temperature", "0.1")))),
            debug=os.getenv("LEDGERLOOP_AI_DEBUG", str(s.get("ai_debug", "false"))).lower() == "true",
            model_overrides={
                "categorize": os.getenv("LEDGERLOOP_AI_MODEL_CATEGORIZE", str(s.get("ai_model_categorize", ""))) or "",
                "merchant": os.getenv("LEDGERLOOP_AI_MODEL_MERCHANT", str(s.get("ai_model_merchant", ""))) or "",
                "anomaly": os.getenv("LEDGERLOOP_AI_MODEL_ANOMALY", str(s.get("ai_model_anomaly", ""))) or "",
            }
        )
  ...
  {{"id": 2, "merchant": "name", "category": "from list", "confidence": 0.8}}
        ...
        try:
            ...
            resp = await service.client.chat.completions.create(
                ...
                messages=[{"role": "user", "content": prompt}],
                temperature=float(os.getenv("LEDGERLOOP_AI_TEMPERATURE", "0.1")),
