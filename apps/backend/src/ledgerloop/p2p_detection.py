...
def run_p2p_detection(limit: int = 10000) -> Dict[str, Any]:
    ...
    This is a synchronous wrapper for the P2P detection pipeline.
    ...
    for row in rows:
        ...
        if result:
            ...
            # Save the P2P detection
            try:
                service.save_p2p_transaction(tx["id"], result)
            except Exception as e:
                logger.warning(f"Failed to save P2P detection for {tx['id']}: {e}")
