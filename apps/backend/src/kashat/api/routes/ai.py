...
from ...ai_service import get_ai_service, TransactionInsights, ping_ai, reset_ai_service, CategorySuggestion
from ..auth import require_admin
from ..limiter import limiter, AI_LIMIT, AI_BULK_LIMIT
...
async def analyze_transaction(request: Request, transaction_id: str) -> TransactionAnalysisResponse:
...
@limiter.limit(AI_LIMIT)
async def normalize_merchant(request: Request, body: MerchantNormalizationRequest) -> dict:
    ...
    insights = await ai_service.analyze_transaction(body.description, amount)
    ...
    if insights.merchant_info:
        ...
        return {
            "original_description": body.description,
            "normalized_name": body.description,
            "confidence": 0.0,
            "merchant_type": None,
...
@limiter.limit(AI_LIMIT)
async def get_category_suggestions(request: Request, transaction_id: str) -> dict:
...
async def _process_bulk_analysis(transaction_data: List[tuple], force_reanalysis: bool = False):
    ...
    try:
        # Process in batches
        import os
        batch_size = int(os.getenv('KASHAT_AI_BATCH_SIZE', '50'))
...
async def _process_bulk_enhancement_with_tracking(job_id: str, transaction_data: List[tuple]):
    ...
    try:
        ...
        import os
        batch_size = settings.get("ai_batch_size") or int(os.getenv('KASHAT_AI_BATCH_SIZE', '20'))
        ...
        logger.error(f"Bulk enhancement error for job {job_id}: {e}", exc_info=True)


@router.get("/suggestions/smart-categories/{transaction_id}")
@limiter.limit(AI_LIMIT)
async def get_smart_category_suggestions(request: Request, transaction_id: str) -> dict:
...
@limiter.limit(AI_BULK_LIMIT)
async def enhance_uncategorized_transactions(request: Request, background_tasks: BackgroundTasks, limit: int = 200, batch_size: int = 10, force: bool = False) -> dict:
    ...
    import os
    os.environ['KASHAT_AI_BATCH_SIZE'] = str(int(batch_size))
...
@limiter.limit(AI_BULK_LIMIT)
async def process_transactions_integrated(request: Request, body: IntegratedProcessRequest = IntegratedProcessRequest()) -> dict:
    ...
    if body.transaction_ids:
        ...
        result = await process_transactions_batch(body.transaction_ids, body.force_reprocess)
    else:
...
@limiter.limit(AI_BULK_LIMIT)
async def run_full_detection_workflow_endpoint(request: Request) -> dict:
...
async def backfill_merchant_memory(limit: int = 500) -> dict:
    ...
    for tx_id, category_id in rows:
        try:
            ...
            errors += 1
            if errors < 5:
                logger.warning(f"Backfill error for {tx_id}: {e}")
