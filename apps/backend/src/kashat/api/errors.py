"""
Standardized Error Handling for Kashat API
...
async def kashat_error_handler(request: Request, exc: Any) -> JSONResponse:
    """Handle KashatError from kashat/errors.py module."""
    request_id = get_request_id(request)

    error_response = ErrorResponse(
        type=exc.code.lower().replace("_", "-"),
        message=exc.message,
        code=exc.code,
        request_id=request_id,
    )
    ...
    logger.warning(
        f"KashatError: {exc.code} - {exc.message}",
        extra={
            "error_code": exc.code,
...
def register_exception_handlers(app: Any) -> None:
    ...
    from ..errors import KashatError

    # Register handlers
    app.add_exception_handler(KashatError, kashat_error_handler)
