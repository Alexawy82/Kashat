...
- Category caching to avoid repeated DB lookups
- Concurrent AI calls with asyncio.gather()
- Batch database writes
- Pre-fetched categorization status checks
"""
...
# =============================================================================
# Category Cache - avoids repeated DB lookups
# =============================================================================
_category_cache: Dict[str, str] = {}  # name_lower -> id
_category_cache_time: float = 0
CATEGORY_CACHE_TTL = 300  # 5 minutes


def _get_category_cache() -> Dict[str, str]:
    """Get or refresh the category cache."""
    global _category_cache, _category_cache_time

    now = time.time()
    if now - _category_cache_time > CATEGORY_CACHE_TTL or not _category_cache:
        conn = get_conn()
        rows = conn.execute("SELECT id, name FROM category").fetchall()
        _category_cache = {row[1].lower(): row[0] for row in rows}
        _category_cache_time = now
        logger.debug(f"Refreshed category cache: {len(_category_cache)} categories")

    return _category_cache


def invalidate_category_cache():
    """Invalidate the category cache (call after category changes)."""
    global _category_cache_time
...
class AutoCategorizationService:
    ...
    async def _learn_pattern_from_ai(self, description: str, category_name: str, confidence: float):
        ...
        try:
            ...
            # Get category ID from cache (fast!)
            cache = _get_category_cache()
            category_id = cache.get(category_name.lower())
    ...
    async def batch_process_uncategorized(self, account_id: Optional[str] = None,
        ...
        """Batch process uncategorized transactions with concurrent AI calls."""
        ...
        # Use 5 concurrent tasks to balance speed vs API rate limits
        sem = asyncio.Semaphore(5)

        async def process_with_semaphore(row):
            async with sem:
                tx_id, description, amount, acc_id = row[0], row[1], row[2], row[3]
                ai_suggestions = row[4] if len(row) > 4 else None
                try:
                    return await asyncio.wait_for(
                        self.process_transaction(tx_id, description, amount, acc_id, ai_suggestions),
                        timeout=35.0  # Per-transaction timeout
                    )
                except asyncio.TimeoutError:
                    logger.warning(f"Transaction {tx_id} timed out")
                    return AutoCategorizationResult(
        ...
        task_results = await asyncio.gather(
            *[process_with_semaphore(row) for row in uncategorized],
        ...
        for i, result in enumerate(task_results):
            row = uncategorized[i]
            tx_id, description, amount = row[0], row[1], row[2]
            ...
            if isinstance(result, Exception):
                ...
                results['details'].append({
                    ...
                    'result': AutoCategorizationResult(
                        transaction_id=tx_id, success=False, action_taken='error', reasoning=str(result)
                    )
                })
                continue
    ...
    def start_categorize_all_job(self, batch_size: int = 50) -> tuple:
        """Start a job to categorize ALL uncategorized transactions.

        Returns (job_id, tx_ids) tuple, or (None, []) if no work to do.

        This method pre-fetches all uncategorized transaction IDs upfront,
        preventing race conditions that occur with repeated queries.
        """
        conn = get_conn()

        # 1. Check if there's already a running categorization job
        existing = conn.execute("""
            SELECT id, status FROM ai_bulk_job
            WHERE job_type = 'auto_categorize_all' AND status = 'processing'
            LIMIT 1
        """).fetchone()

        if existing:
            logger.warning(f"Categorization job {existing[0]} already running")
            return (existing[0], [])  # Return existing job ID

        # 2. Fetch ALL uncategorized transaction IDs in one query
        uncategorized_ids = conn.execute("""
        ...
        tx_ids = [row[0] for row in uncategorized_ids]
        if not tx_ids:
        ...
        conn.execute("""
            ...
            VALUES (?, 'auto_categorize_all', ?, 'pending', ?)
        """, [job_id, len(tx_ids), datetime.now(UTC).isoformat()])

        logger.info(f"Created categorization job {job_id} for {len(tx_ids)} transactions")
        return (job_id, tx_ids)

    async def process_categorization_job(self, job_id: str, tx_ids: List[str], batch_size: int = 50):
        """Process a categorization job with concurrent AI calls.

        Processes pre-determined transaction IDs in batches with concurrent
        processing within each batch. Updates progress after each batch.
        """
        conn = get_conn()
        ...
        conn.execute("UPDATE ai_bulk_job SET status = 'processing' WHERE id = ?", [job_id])
        logger.info(f"Starting categorization job {job_id} with {len(tx_ids)} transactions")

        processed = 0
        categorized = 0
        errors = 0

        # Semaphore for concurrent processing (5 concurrent tasks)
        sem = asyncio.Semaphore(5)

        async def process_one(tx_id: str, desc: str, amount: float, acc_id: str):
            async with sem:
                try:
                    return await asyncio.wait_for(
                        self.process_transaction(tx_id, desc, amount, acc_id),
                        timeout=35.0
                    )
                except asyncio.TimeoutError:
                    return AutoCategorizationResult(
                        transaction_id=tx_id, success=False,
                        action_taken='error', reasoning='Timeout'
                    )
                except Exception as e:
                    return AutoCategorizationResult(
                        transaction_id=tx_id, success=False,
        ...
        for i in range(0, len(tx_ids), batch_size):
            ...
            status_row = conn.execute(
                "SELECT status FROM ai_bulk_job WHERE id = ?", [job_id]
            ).fetchone()
            if status_row and status_row[0] in ('cancelled', 'paused'):
                logger.info(f"Job {job_id} was {status_row[0]}, stopping")
                break

            # Fetch transaction data for this batch
            placeholders = ','.join(['?' for _ in batch_ids])
            batch_data = conn.execute(f"""
                SELECT id, description_norm, amount, account_id
                FROM [transaction] WHERE id IN ({placeholders})
            """, batch_ids).fetchall()

            # Process batch concurrently
            tasks = [
                process_one(row[0], row[1], row[2], row[3])
            ...
            for result in batch_results:
                if isinstance(result, Exception):
                    ...
                    processed += 1
                    if result.action_taken in ('applied', 'created_and_applied'):
                        categorized += 1
            ...
            conn.execute("""
                ...
                WHERE id = ?
            """, [processed, categorized, job_id])

            # Log progress
            pct = round((i + len(batch_ids)) / len(tx_ids) * 100, 1)
            logger.info(f"Job {job_id}: {i + len(batch_ids)}/{len(tx_ids)} ({pct}%) - {categorized} categorized")
        ...
        conn.execute("""
            ...
            SET status = ?, completed_at = ?,
                ...
                error_message = ?
            WHERE id = ?
        """, [final_status, datetime.now(UTC).isoformat(), processed, categorized, error_msg, job_id])

        logger.info(f"Completed job {job_id}: {categorized}/{processed} categorized, {errors} errors")
...
def start_categorize_all_job(batch_size: int = 50) -> tuple:
    """Start a job to categorize all uncategorized transactions.

    Returns (job_id, tx_ids) tuple. Use with process_categorization_job().
    """
    service = get_auto_categorization_service()
    return service.start_categorize_all_job(batch_size)


async def process_categorization_job(job_id: str, tx_ids: List[str], batch_size: int = 50):
    """Process a categorization job - run as background task."""
    service = get_auto_categorization_service()
    await service.process_categorization_job(job_id, tx_ids, batch_size)
