...
# Add the src directory to the path so we can import kashat modules
sys.path.insert(0, str(Path(__file__).parent.parent))

from kashat.db import connect, get_conn
from kashat.ingest_csv import import_csv_upload
from kashat.ingest_pdf import import_pdf_upload
from kashat.transfers import detect_transfers
from kashat.recurring import detect_recurring_series
from kashat.ai_service import get_ai_service
from kashat.rules import apply_all_rules


@click.group()
@click.option('--data-dir', envvar='KASHAT_DATA_DIR', 
...
def cli(ctx, data_dir, verbose):
    ...
    if data_dir:
        os.environ['KASHAT_DATA_DIR'] = data_dir
...
def health(ctx):
    ...
    try:
        ...
        env_vars = ['KASHAT_DATA_DIR', 'KASHAT_API_TOKEN']
        for var in env_vars:
            value = os.getenv(var)
            if value:
                if verbose:
