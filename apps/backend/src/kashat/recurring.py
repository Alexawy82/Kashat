...
def _parse_date(d) -> date:
    """Parse a date from various formats (string, date, datetime)."""
...
def make_tx(row: tuple) -> Tx:
    """Create a Tx from a database row, handling date parsing."""
    return Tx(row[0], row[1], _parse_date(row[2]), row[3], row[4])
...
_RECURRING_COLUMNS_ENSURED = False

def _ensure_recurring_series_columns(conn) -> None:
    global _RECURRING_COLUMNS_ENSURED
    if _RECURRING_COLUMNS_ENSURED:
        return
    for sql in (
        ...
        try:
            ...
            pass
    _RECURRING_COLUMNS_ENSURED = True
...
def suggest_recurring(
    ...
    # P2P-FIRST PIPELINE: Exclude transactions already linked to P2P recurring series
    # This prevents double-counting between P2P detection and standard recurring detection
    p2p_exclusion = """
        AND t.id NOT IN (
            SELECT p.tx_id FROM p2p_transaction p
